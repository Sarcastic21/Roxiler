...
export default function SuperAdminDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      const data = await superAdminGetUsers(token);
      setUsers(data);
    } catch (error) {
  ...
  const handleUserSubmit = async e => {
    ...
    setLoading(true);
    try {
      const data = await superAdminCreateUser(form, token);
  ...
  const handleDeleteUser = async id => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        await superAdminDeleteUser(id, token);
  ...
  const getRoleColor = (role) => {
    switch (role) {
      case "super_admin": return "#FFD700";
    ...
    <div className="super-admin-dashboard">
      ...
      <div className="sidebar">
        <div className="sidebar-header">
          ...
          <p className="admin-role">Super Admin Dashboard</p>
        </div>
  ...
  <p className="note-text">
    <strong>Note:</strong> Please create a <span className="highlight">store owner</span> first.  
    Once the owner exists, you can find their <span className="highlight">Owner ID</span> in the Users list.  
    Use this Owner ID when registering a new store.
  </p>
</div>
